...
import { deleteCommentById } from '@/features/comments/comment-actions';
...
export const CommentItem = ({ comment, postId }: Props) => {
  ...
  const canDelete = session?.user.name === comment.author.username;
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <span className="font-medium text-foreground">{comment.author.username || 'Anonymous'}</span>
          <span>·</span>
